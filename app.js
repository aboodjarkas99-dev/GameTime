...
function orderSig(list){return JSON.stringify(list.map(o=>[o.id,o.version,o.batchStatus,o.prodStatus,o.batchChecked,o.prodChecked,o.held,o.actualQuart,o.actualGallon,o.actualFive,o.actualJerry,o.jerryEnabled,o.jerry,o.batchOrder,o.prodOrder,o.holdLine,o.productionOnly,o.carryoverFrom,o.date,o.deletedAt]))}
...
function card(o,dept,i){
  ...
  return '<article class="card"'+drag+'>'+
    ...
    '<div class="actions"><button class="start '+(checked?'':'locked')+'" data-action="start" data-id="'+o.id+'" data-dept="'+dept+'">▶ START WORK</button><button class="finish '+(checked?'':'locked')+'" data-action="done" data-id="'+o.id+'" data-dept="'+dept+'">✓ WORK DONE</button><button class="hold" data-action="hold" data-id="'+o.id+'">'+(o.held?'RESUME WORK':'PUT ON HOLD')+'</button>'+(dept==='prod'&&st!=='done'?'<button class="carrynext" data-action="carry" data-id="'+o.id+'" data-dept="prod">↪ MOVE REMAINDER TO NEXT DAY</button>':'')+'</div>'+
...
function render(){
  ...
  const day=currentDay(),batch=[...day].filter(o=>!o.productionOnly).sort((a,b)=>a.batchOrder-b.batchOrder),prod=[...day].sort((a,b)=>a.prodOrder-b.prodOrder),visible=view==='batch'?batch:view==='prod'?prod:day;
...
async function handleAction(btn){
  ...
  try{
    ...
    if(btn.dataset.action==='carry'){openCarry(id);return}
...
function dayState(d){
  const arr=orders.filter(o=>!o.holdLine&&o.date===d&&(view!=='batch'||!o.productionOnly));if(!arr.length)return'emptyday';
  const complete=arr.every(orderCompleteForView);return complete?'good':'bad'
}
function openMonth(){monthCursor=new Date(selected.getFullYear(),selected.getMonth(),1);showModal('monthModal');renderMonth()}
function orderCompleteForView(o){return view==='batch'?o.batchStatus==='done':view==='prod'?o.prodStatus==='done':o.productionOnly?o.prodStatus==='done':o.batchStatus==='done'&&o.prodStatus==='done'}
function renderMonth(){
  ...
  for(let i=0;i<42;i++){
    ...
    const arr=orders.filter(o=>!o.holdLine&&o.date===di&&(view!=='batch'||!o.productionOnly)).sort((a,b)=>(view==='prod'?a.prodOrder:a.batchOrder||999)-(view==='prod'?b.prodOrder:b.batchOrder||999));
    const chips=arr.map(o=>'<div class="calorder '+(orderCompleteForView(o)?'calgood':'calbad')+'" title="'+esc(o.product)+'">'+(o.productionOnly?'↪ ':'')+esc(o.product)+(o.batch?' <span>'+esc(o.batch)+'</span>':'')+'</div>').join('');
